                        ...
                        <tr v-for="(row, rowIndex) in filteredItems" :key="rowIndex"
                            ...
                            <th v-for="(cell, cellIndex) in Object.values(row ? row : [])" :key="cellIndex" scope="row"
                                :class="[
                                    ...
                                    `${columns[cellIndex].isShowing ? 'px-6 py-1' : ''}`
                                ]">
...
import ModalPendiente from '../componentes/CustomModalUpdatePendiente.vue';
...
const search = (e: any) => {
    ...
    currentPage.value = 1; // Resetear a la primera página al realizar una nueva búsqueda
};
    ...
    if (props.data && props.data.length > 0 && props.data[0]) {
        columns.value = [...Object.keys(props.data[0] as Record<string, unknown>).map((col) => {
            ...
            if (col === 'TITU_BIBLIOTECA_OBSERVACION')
                col = 'Observaciones'
            return { head: col, isShowing: col === 'Senescyt' ? false : col === 'Biblioteca PDF' ? false : true }
        }),
